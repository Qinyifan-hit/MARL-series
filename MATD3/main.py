# Multi-Agent TD3 (2018-Addressing Function Approximation Error in Actor-Critic Methods)
import torch
import numpy as np
...
parser.add_argument("--noise_decay_steps", type=int, default=int(3e5), help="Number of step to decay noise to minnoise")
...
def main():
    ...
    env_train = ENV_MAP[opt.env_name].parallel_env(N=3, max_cycles=opt.max_steps, continuous_actions=True,
                                                   render_mode='human' if opt.render else None)
    env_eval = ENV_MAP[opt.env_name].parallel_env(N=3, max_cycles=opt.max_steps, continuous_actions=True,
                                                  render_mode='human' if opt.render else None)
    ...
    for agent_id in range(opt.Num_agent):
        ...
        models.append(TD3_Agent(**vars(opt)))
    if opt.Loadmodel:
        for agent_id in range(opt.Num_agent):
            models[agent_id].load(opt.BName, agent_id, opt.load_Idex)
    ...
    if opt.render:
        while True:
            env_show = ENV_MAP[opt.env_name].parallel_env(N=3, max_cycles=opt.max_steps, continuous_actions=True,
                                                          render_mode='human' if opt.render else None)
        ...
        while steps <= opt.total_steps:
            ...
            while not done:
                ...
                if steps < opt.warmup_steps:
                    ...
                    action_list = [models[a_id].action_select(state_list[a_id], False) for a_id in range(opt.Num_agent)]
                ...
                if steps >= opt.warmup_steps and steps % opt.update_every == 0:
                    for agent_id in range(opt.Num_agent):
                        s, a, r, s_, dw = Reply.B_sample(opt.batch_size)
                        a_ = [models[a_id].A_target(s_[a_id]) for a_id in range(opt.Num_agent)]
                        models[agent_id].train(s, a, r, s_, dw, a_, agent_id)
